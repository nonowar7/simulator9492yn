...
int ShuntingYard::doubleLength(double number) {
    ...
    // for integer, reduncing the decimal point count
    if (integerInd) {
...
int ShuntingYard::handleVars(const string &str, map<string, Expression *> vars) {
    string temp = str;
    string var;
    int varLength=0;
    ...
    while ( ((temp.at(0) < LETTER_TOP-CAPITAL_DIFFER) && (temp.at(0) > LETTER_BOTTOM-CAPITAL_DIFFER)) ||
        ...
        if ((isOperator(temp.at(1)) || temp.at(1) == '(' || temp.at(1) == ')') && vars.count(var) > 0 ) {
            return varLength;
        }
...
Expression* ShuntingYard::calculateSYard(const string& str, map<string, Expression*> vars) {
    ...
    while (!temp.empty()) {
         ...
         * to the queue.
         */
        int varLength=handleVars(temp, vars);
        if (varLength >0) {
    ...
    return makeExpression(vars);


}

Expression* ShuntingYard::makeExpression(map<string, Expression*> vars) {
    ...
    while (!theQueue.empty()) {
        ...
        stackExp.push(new Var(exp, vars));
